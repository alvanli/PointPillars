...
from .utils import boxes_iou3d_gpu, logger, BATCH_SIZE
...
class ProposalTargetLayer(nn.Module):
    def __init__(self, roi_sampler_cfg):
        ...
        self.REG_FG_THRESH = self.roi_sampler_cfg.REG_FG_THRESH
        self.CLS_FG_THRESH = self.roi_sampler_cfg.CLS_FG_THRESH
        self.CLS_BG_THRESH = self.roi_sampler_cfg.CLS_BG_THRESH
        self.CLS_SCORE_TYPE = self.roi_sampler_cfg.CLS_SCORE_TYPE
        self.ROI_PER_IMAGE = self.roi_sampler_cfg.ROI_PER_IMAGE
        self.FG_RATIO = self.roi_sampler_cfg.FG_RATIO
        self.CLS_BG_THRESH_LO = self.roi_sampler_cfg.CLS_BG_THRESH_LO
        self.HARD_BG_RATIO = self.roi_sampler_cfg.HARD_BG_RATIO
    ...
    def sample_rois_for_rcnn(self, batch_dict: Dict[str, torch.Tensor]):
        """
        Args:
            batch_dict:
                batch_size:
    ...
    def get_max_iou_with_same_class(self, rois, roi_labels, gt_boxes, gt_labels):
        ...
        from_range = int(gt_labels.min().item())
        to_range = int(gt_labels.max().item() + 1)
