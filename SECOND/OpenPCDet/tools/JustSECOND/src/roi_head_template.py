...
from .utils import ResidualCoder, rotate_points_along_z, WeightedSmoothL1Loss, get_corner_loss_lidar
from .utils import class_agnostic_nms, BATCH_SIZE
...
class RoIHeadTemplate(nn.Module):
    ...
    def build_losses(self, losses_cfg):
        self.add_module(
            'reg_loss_func',
            WeightedSmoothL1Loss(code_weights=losses_cfg.LOSS_WEIGHTS['code_weights'])
        )
    ...
    @torch.no_grad()
    def proposal_layer(self, batch_dict: Dict[str, torch.Tensor], nms_config):
        """
        Args:
            batch_dict:
                batch_size:
