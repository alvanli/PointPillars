...
class NuScenesDataset(DatasetTemplate):
    ...
    def __getitem__(self, index):
        ...
        #     atmos_model = np.random.choice(['rain', 'chu_hogg_fog', 'moderate_advection_fog']) # 'snow', 
        #     self.logger.info('Augmenting with {}'.format(atmos_model))
        #     lisa = LISA(atm_model=atmos_model)
        #     if atmos_model not in ["chu_hogg_fog", "moderate_advection_fog"]:
        #         new_points = lisa.augment(points[:, :4], np.random.randint(5, 13))
        #     else:
